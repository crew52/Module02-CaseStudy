...
class CheckingAccountTest {
    ...
    void testTransferValidAmount() {
        ...
        assertEquals(1, account1.getTransactions().size(), "Number of transactions in Account 1");
        assertEquals(1, account2.getTransactions().size(), "Number of transactions in Account 2");

        // Kiểm tra giao dịch trong tài khoản nguồn
        assertInstanceOf(TransferTransaction.class, account1.getTransactions().get(0));
        TransferTransaction transferTransaction1 = (TransferTransaction) account1.getTransactions().get(0);
        assertEquals(100.0, transferTransaction1.getAmount());
        assertEquals(account2.getAccountId(), transferTransaction1.targetAccountId);

        // Kiểm tra giao dịch trong tài khoản đích
        assertInstanceOf(TransferTransaction.class, account2.getTransactions().get(0));
        TransferTransaction transferTransaction2 = (TransferTransaction) account2.getTransactions().get(0);
        assertEquals(100.0, transferTransaction2.getAmount());
        assertEquals(account1.getAccountId(), transferTransaction2.targetAccountId);
    }
}